...
export async function tbServerGetRowOkKeys() {
  const supabase = await getClient();
  if (!supabase) {
    return { ok: false, message: 'Chưa cấu hình Supabase.', keys: [] };
  }
  const { data, error } = await supabase
    .from('sp2_port_cache')
    .select('data')
    .eq('cache_key', TB_ROW_OK_CACHE_KEY)
    .maybeSingle();
  if (error) {
    return { ok: false, message: error.message || 'Lỗi đọc trạng thái OK.', keys: [] };
  }
  const payload = unwrapJsonbData(data?.data);
...
export async function tbServerMarkRowOk(row) {
  ...
  const { data: found, error: readErr } = await supabase
    ...
    .maybeSingle();
  if (readErr) {
    return { ok: false, message: readErr.message || 'Lỗi đọc trạng thái OK.' };
  }
  const prev = unwrapJsonbData(found?.data) || {};
  ...
  const { error: upErr } = await supabase
    .from('sp2_port_cache')
    .upsert({ cache_key: TB_ROW_OK_CACHE_KEY, data: payload, updated_at: now }, { onConflict: 'cache_key' });
  if (upErr) {
    return { ok: false, message: upErr.message || 'Không lưu được trạng thái OK.' };
  }
