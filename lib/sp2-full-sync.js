...
export async function runFullSp2Sync({ auth, signal, onProgress, delayMs = 100, server = null }) {
  ...
  onProgress?.({ phase: 'scan', done: 0, total: 0, label: 'Đang quét danh mục (Tổ KT → … → Port)…' });
  ...
  if (signal.aborted) {
    if (serverMode) {
      ...
      await setSyncMeta({ lastSyncAborted: true, lastSyncAt: Date.now(), lastSyncTotal: 0, lastSyncErrors: 0 });
    }
    return { aborted: true, errors: 0, total: 0, completed: 0, server: serverMode };
  }

  onProgress?.({ phase: 'tracuu', done: 0, total: tasks.length, label: `Bắt đầu tra cứu ${tasks.length} port…` });
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    const t = tasks[i];
    completed = i + 1;
    onProgress?.({ phase: 'tracuu', done: completed, total: tasks.length, label: t.label });
  ...
  return { aborted: signal.aborted, errors, total: tasks.length, completed, server: serverMode };
}
