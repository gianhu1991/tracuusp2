...
export async function sp2ServerTruncate() {
  ...
  // Chỉ xóa dữ liệu cache SP2; giữ nguyên cache thuê bao/lịch sử chuyển địa bàn.
  const { error: delErr } = await supabase
    .from('sp2_port_cache')
    .delete()
    .not('cache_key', 'like', `${TB_SHARED_PREFIX}%`)
    .not('cache_key', 'like', `${TB_TRANSFER_PREFIX}%`);
  if (delErr) {
    return { ok: false, message: delErr.message || 'Không xóa được bảng cache.' };
  }
    ...
    for (const row of data) {
      ...
      for (const item of list) {
        ...
        for (const target of queryTargets) {
          // Match an toan:
          // - Uu tien exact sau khi normalize.
          // - Cho phep contains 1 chieu (gia tri cache chua input) de ho tro input rut gon.
          // - KHONG dung contains nguoc (input chua cache) vi de gay trung sai.
          const exactMatch = (
