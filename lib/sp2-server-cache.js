...
import { matchS2Query } from './s2-match';
...
/** Dung lượng / đã dùng / còn trống từ bản ghi splitter OneBSS. */
export function capacityFromSplitterItem(item) {
  ...
  return { dungLuong, daDung, chuaDung };
}
...
export async function sp2ServerGetS2CapacityRows() {
  ...
  for (const row of listed.rows) {
      ...
      for (const item of list) {
        ...
        const { dungLuong, daDung, chuaDung } = capacityFromSplitterItem(item);
...
export async function sp2ServerLookupS2Rows(inputCodes = [], options = {}) {
  const fuzzyMatch = Boolean(options.fuzzyMatch);
  const listed = await listAllCacheRows();
  if (!listed.ok) {
  ...
  const queryTargets = normalizedInputs.map((raw) => ({ raw }));
  ...
  for (const row of listed.rows) {
      ...
      for (const item of list) {
        ...
        if (!kyHieu && !tenSplitter) continue;

        for (const target of queryTargets) {
          const { match, matchType } = matchS2Query({
            query: target.raw,
            kyHieu,
            tenSplitter,
            fuzzyMatch,
  ...
  for (const list of byQuery.values()) {
    if (!fuzzyMatch) {
      bestRows.push(...list.filter((item) => String(item?.matchType || '') === 'exact'));
      continue;
    }
